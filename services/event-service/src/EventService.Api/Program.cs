...
using EventService.Api.Logging;
using EventService.Api.Middleware;
using EventService.Api.OpenApi;
...
builder.Host.UseSerilog((context, services, loggerConfiguration) =>
{
    ...
    // Never serialize AWS credentials into logs.
...
builder.Services.AddRateLimiter(options =>
{
    ...
    options.AddPolicy("user-or-ip", httpContext =>
    {
        ...
        return RateLimitPartition.GetFixedWindowLimiter(key, _ => new System.Threading.RateLimiting.FixedWindowRateLimiterOptions
        {
            PermitLimit = 100,
...
builder.Services.AddAuthorization(ApiPolicies.AddEventServiceAuthorization);
...
builder.Services.AddEventServiceApplication();
...
{
    var seeder = scope.ServiceProvider.GetRequiredService<EventDatabaseSeeder>();
...
app.MapGet("/", () => Results.Ok(new { service = "EventService", docs = "/scalar/v1", health = "/health/live" }));
